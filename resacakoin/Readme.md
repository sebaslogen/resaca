...
[![Supported Compose Platforms](https://img.shields.io/badge/Platforms-Android_|_iOS-blue)](https://img.shields.io/badge/Platforms-Android_|_iOS-blue)
...
<details>
  <summary>Scope a ViewModel injected by Koin with a clear delay to a Composable</summary>
...
fun DemoInjectedViewModelWithClearDelay() {
    ...
    val myInjectedViewModel: MyViewModel = koinViewModelScoped(clearDelay = 5.seconds)
    DemoComposable(viewModel = myInjectedViewModel)
}
    ...
    LazyColumn() {
        items(items = listItems, key = { it }) { item ->
            val myScopedVM: MyViewModel = if (item == 1) {
                ...
                koinViewModelScoped(key = item, clearDelay = 5.seconds)
            } else {
                // These items' ViewModels stay alive as long as the item is in the list
                koinViewModelScoped(key = item, keyInScopeResolver = keys)
            }
            DemoComposable(inputObject = myScopedVM)
        }
    }
...
When you declare the ViewModel factory in your Koin Module using the `viewModel {}` or the `factory {}` syntax, then 
