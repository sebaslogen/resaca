...
class KeyInScopeResolverTest : ComposeTestUtils {
    ...
    fun `Given a long lazy list when the first item is not visible anymore, then its ViewModel remains in the container and it's not cleared`() = runTest {
                ...
                LazyColumn(modifier = Modifier.fillMaxHeight()) {
                    items(items = listItems, key = { it.number }) { item ->
                        Box(modifier = Modifier.size(width = 200.dp, height = 100.dp)) {
                            val fakeScopedVM: FakeMetroInjectedViewModel =
                                metroViewModelScoped(
                                    key = item,
                                    keyInScopeResolver = keys,
                                    factory = createMetroAssistedFactory(ResacaSampleApp.metroGraph, item.number),
                                )
                            DemoComposable(inputObject = fakeScopedVM, objectType = "FakeMetroInjectedViewModel $item", scoped = true)
                        }
                    }
                }
            }
        ...
        onNodeWithTestTag("FakeMetroInjectedViewModel 1 Scoped").assertExists() // Required to trigger recomposition
        ...
        onNodeWithTestTag("FakeMetroInjectedViewModel 1 Scoped").assertExists()
        onNodeWithTestTag("FakeMetroInjectedViewModel 5 Scoped", assertDisplayed = false).assertIsNotDisplayed() // Required to trigger recomposition
        assert(finalAmountOfViewModelsCleared == initialAmountOfViewModelsCleared) {
            "The amount of FakeMetroInjectedViewModels that were cleared after change ($finalAmountOfViewModelsCleared) " +
                    "was not the same that the amount before the change ($initialAmountOfViewModelsCleared). It should be 0"
        }
    }

    @Test
    fun `Given a long lazy list when an item is removed from the list, then its ViewModel is cleared from the container`() = runTest {
        ...
        onNodeWithTestTag("FakeMetroInjectedViewModel 2 Scoped").assertExists() // Required to trigger recomposition
        advanceTimeBy(1000) // Advance time to allow clear call on ScopedViewModelContainer to be processed before querying the counter
        printComposeUiTreeToLog()

        // Then one scoped ViewModel is cleared
        val finalAmountOfViewModelsCleared = viewModelsClearedGloballySharedCounter.get()
        onNodeWithTestTag("FakeMetroInjectedViewModel 2 Scoped").assertExists()
        onNodeWithTestTag("FakeMetroInjectedViewModel 1 Scoped", assertDisplayed = false).assertIsNotDisplayed() // Required to trigger recomposition
        assert(finalAmountOfViewModelsCleared == initialAmountOfViewModelsCleared + 1) {
            "The amount of FakeMetroInjectedViewModels that were cleared after keys list change ($finalAmountOfViewModelsCleared) " +
