...
class ScopeKeysTest : ComposeTestUtils {
    ...
    @Test
    fun `when the key used for metroViewModelScoped changes, then the scoped ViewModel is different`() {

        // Given the starting screen with scoped object that uses a key
        composeTestRule.activity.setContent {
            var myKey by remember { mutableStateOf(false) }
            val fakeInjectedViewModel: FakeMetroInjectedViewModel =
        ...
        val initialFakeInjectedViewModelText = retrieveTextFromNodeWithTestTag("FakeMetroInjectedViewModel Scoped")
        ...
        onNodeWithTestTag("FakeMetroInjectedViewModel Scoped").assertIsDisplayed()
